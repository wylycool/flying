...
def user_pass_forget(code=None):
    ...
    return render_template('user/forget.html', page_name='user', ver_code=ver_code['question'], code=code, has_code=has_code, user=user,title='找回密码')
...
def send_active_email(username, user_id, email, is_forget=False):
    ...
    if is_forget:
        body = render_template('email/user_repwd.html',username=username,url=url_for('user.user_pass_forget', code=code.inserted_id, _external=True))
        return utils.send_email(email, '重置密码', body=body)
        # return
    body = render_template('email/user_activate.html', username=username,url=url_for('user.user_active', code=code.inserted_id, _external=True))
    return utils.send_email(email, '账号激活', body=body)
    ...
    if request.method == 'GET':
        ...
        if code:
            ...
            if user_id:
                ...
                #这儿的user是一个字典, 存储着用户的所有属性,不是一个对象
                # print(user) #可以在终端打印出全部字典
                # print(user['email']) #打印字典中email的值,实际上就是用户user的属性
                return render_template('user/activate.html', user=user,title='激活')
        if not current_user.is_authenticated:
            abort(403)
...
def register():
    ...
    return render_template('user/reg.html', ver_code=ver_code['question'], form=user_form,title='注册')
...
def login():
    ...
    if user_form.is_submitted():
        if not user_form.validate():
            #直接返回json格式, 浏览器的响应头中的content-type会显示为application/json
            return jsonify(models.R.fail(code_msg.PARAM_ERROR.get_msg(), str(user_form.errors)))
        utils.verify_num(user_form.vercode.data)
        #从mongodb数据库中拿出一个数据
        user = mongo.db.users.find_one({'email': user_form.email.data})
...
#可以捕获全局的404异常, 并渲染友好界面
@user_view.app_errorhandler(404)
def page_not_found(e):
    return render_template('other/404.html'),404

@user_view.app_errorhandler(500)
def page_not_found(e):
    return render_template('other/404.html'),500